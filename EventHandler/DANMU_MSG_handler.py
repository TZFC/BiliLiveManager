...
async def handle_danmu_msg(event, database, master_config, live_room, room_config):
    ...
    async with asyncio.TaskGroup() as tg:
        ...
        if received_uid in {room_config['master_credential'].dedeuserid, streamer_uid}:
            if "checkin" in text:
                if room_config["feature_flags"]["checkin"] and (room_config['state']['pre-checkin'] is False):
                    ...
                    tg.create_task(send_report_checkin(live_room=live_room, top_uid_count=top_uid_count))
